...
const state = { devices: [], filters: { search: "", type: "All", room: "All", status: "All", validation: "All" } };
...
function getUserId() { return getUser().id || getUser()._id || ""; }
...
function loadUserBadge() {
  const user = getUser();
  if ($("userBadge")) $("userBadge").textContent = user.name ? `👤 ${user.name}` : "👤 User";
}
function logout() {
